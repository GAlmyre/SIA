...
void renderBlock(Scene* scene, ImageBlock& block)
{
    ...
    auto sampleCount = camera->sampleCount();
    float a = 0;
    float b = 0;
    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
          ...
          Ray ray;
          Color3f meanColor = Color3f(0,0,0);
          ...
          for (int j = 0; j < sampleCount; j++) {
            for (int i = 0; i < sampleCount; i++) {
              if (camera->isStratified()) {
                float low = 0;
                float high = 1;
                a = Eigen::internal::random<float>(low,high)/sampleCount;
                b = Eigen::internal::random<float>(low,high)/sampleCount;
              }
              ray.direction = (camF + camX * (2.0*float(x + (float)i/sampleCount + a + offset[0])/float(camera->vpWidth())  - 1.)
                                    - camY * (2.0*float(y + (float)j/sampleCount + b + offset[1])/float(camera->vpHeight()) - 1.)).normalized();

              meanColor = meanColor + integrator->Li(scene, ray);
            }
          }
          meanColor = meanColor/(sampleCount*sampleCount);
          block.put(Vector2f(x+offset[0], y+offset[1]), meanColor);
        }
    }
}
