        ...
        if (hit.foundIntersection())
        {
            ...
            for(LightList::const_iterator it=lights.begin(); it!=lights.end(); ++it)
            {
                ...
                if (dynamic_cast<const AreaLight*>(*it)) {
                  ...
                  + light->uVec() * Eigen::internal::random<float>(-0.5, 0.5)*light->size()[0]
                  + light->vVec() * Eigen::internal::random<float>(-0.5, 0.5)*light->size()[1];
                  intensity = light->intensity(pos, lightDir+pos);
                  dist = lightDir.norm();
                  lightDir.normalize();
                }
                else {
