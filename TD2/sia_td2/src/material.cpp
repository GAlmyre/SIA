...
Vector3f Material::us(const Vector3f& normal, float& pdf) const
{
  Vector3f u = normal.unitOrthogonal();
  Vector3f v = normal.cross(u);

  Vector3f d = Warp::squareToUniformHemisphere(Point2f(Eigen::internal::random<float>(0,1),Eigen::internal::random<float>(0,1)));
  pdf = Warp::squareToUniformHemispherePdf(d);

  return d.x() * u + d.y() * v + d.z() * normal;
}
