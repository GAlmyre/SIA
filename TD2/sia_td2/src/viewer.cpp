    ...
    else if(m_scene) // scene loaded, OpenGL rendering
    {
        ...
        if(m_drawRay)
        {
            ...
            if(m_hit.foundIntersection())
            {
                ...
                glUniform3f(m_flatProgram.uniform("color"),1.f,0.f,0.f);
                Line::draw(&m_flatProgram, x, x + 0.25f * normal);
                glUniform3f(m_flatProgram.uniform("color"),0.f,1.f,0.f);
                for (int i = 0; i < 100; i++) {
                  float fi = i;


                  Line::draw(&m_flatProgram, x, x + 0.25f * m_hit.shape()->material()->is(normal, -m_ray.direction));
                }
