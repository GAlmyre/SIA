...
public:
    ...
    virtual Vector3f is(const Vector3f& normal, const Vector3f& viewDir) const { return Vector3f(0);};
    virtual Color3f premultBrdf(const Vector3f& viewDir, const Vector3f& lightDir, const Normal3f& normal, const Vector2f& uv) const {
      return Color3f(0.f);
    }
    Vector3f us(const Vector3f& normal, float& pdf) const;
protected:
    /// texture
...
public:
    ...
    Color3f brdf(const Vector3f& viewDir, const Vector3f& lightDir, const Normal3f& normal, const Vector2f& uv) const;
     virtual Color3f premultBrdf(const Vector3f& viewDir, const Vector3f& lightDir, const Normal3f& normal, const Vector2f& uv) const;
    std::string toString() const;
    virtual Vector3f is(const Vector3f& normal, const Vector3f& viewDir) const override;
